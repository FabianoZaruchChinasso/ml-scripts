...
- fill-last: fill empty (NaN) values in a column or list of columns with the last non-empty value (forward fill). If the first row is empty, use the optional directive value as the default fallback. When `values` is omitted, it performs global forward fill (legacy behavior). When `values` is provided as `group_column,group_name` or `group_column::group_name`, the fill is restricted to rows from that group only. An optional third value in comma format (`group_column,group_name,default`) is used as the fallback default for the first empty row of that group.
- fill-last-auto: fill empty (NaN) values in a column or list of columns using the last non-empty value from previous rows that belong to the same group as the current row. The `values` field must contain the group column name; the group value is automatically taken from each current row. If there is no previous valid value for the group, fill with `0`.
- math: add a new column or a list of new columns to the dataset. The values are calculated according to one or more arithmetic formulas provided in the `values` field (separated by commas if multiple). If no values are provided, the operation evaluates arithmetic expressions already stored in the target cell values and replaces them in place. Formulas support standard operators (`+`, `-`, `*`, `/`), parentheses for precedence, power operator (`**`), and advanced mathematical functions (e.g., `log`, `sqrt`, `exp`, `sin`, `cos`). Formulas can reference existing columns by name or use constants.
...
- suppress-empty-pct: suppress (remove) rows from the dataset when the percentage of empty cells in the specified columns is greater than the provided threshold (value between 0 and 1).
